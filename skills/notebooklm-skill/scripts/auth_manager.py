...
class AuthManager:
    ...
    def setup_auth(self, headless: bool = False, timeout_minutes: int = 10) -> bool:
        ...
        try:
            ...
            from config import NOTEBOOKLM_EMAIL, NOTEBOOKLM_PASSWORD
            
            if NOTEBOOKLM_EMAIL and NOTEBOOKLM_PASSWORD:
                print(f"  📧 Using configured account: {NOTEBOOKLM_EMAIL}")
                
                try:
                    # Wait for and fill email field
                    page.wait_for_selector('input[type="email"]', timeout=10000)
                    page.fill('input[type="email"]', NOTEBOOKLM_EMAIL)
                    page.click('button:has-text("Next"), #identifierNext')
                    
                    # Wait for and fill password field
                    page.wait_for_selector('input[type="password"]', timeout=10000)
                    page.fill('input[type="password"]', NOTEBOOKLM_PASSWORD)
                    page.click('button:has-text("Next"), #passwordNext')
                    
                    print("  ⏳ Waiting for login to complete...")
                    
                except Exception as e:
                    print(f"  ⚠️ Auto-login failed, switching to manual: {e}")
                    # Fall through to manual login
            else:
                print("  ⚠️ No credentials found in .env, waiting for manual login...")
                print("  💡 Tip: Set NOTEBOOKLM_EMAIL and NOTEBOOKLM_PASSWORD in ~/.openclaw/workspace/.env")
